...
void DX12Renderer::Initialize(void* windowHandle, uint32_t width, uint32_t height) {
    ...
    m_graphicsMemory = MakeUnique<DirectX::GraphicsMemory>(m_device.Get());
    m_commonStates = MakeUnique<DirectX::CommonStates>(m_device.Get());
...
void DX12Renderer::CreateDescriptorHeaps() {
    m_rtvHeap = MakeUnique<DirectX::DescriptorHeap>(
        m_device.Get(),
